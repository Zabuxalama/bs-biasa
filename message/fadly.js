...
case 'miaw':{
            ...
            if (!q) return reply(`Penggunaan ${command} halo`)
            axios.get(`https://api-sv2.simsimi.net/v2/?text=${q}&lc=id&cf=false`)
                .then(({ data }) => reply(`${data.success}`))
...
${prefix}nuliskiri Jangan Lupa Donasi`)
        ...
        case prefix+'miaw':{
            if (!isPremium) return reply(mess.OnlyPrem)
            if (!q) return reply(`Penggunaan ${command} hallo`)
...
Nick : ${data.result}`
        ...
        case prefix+'setlogo':
            if (!isOwner) return reply(mess.OnlyOwner)
            if (!isQuotedImage) return reply('Reply imagenya goblokk!')
